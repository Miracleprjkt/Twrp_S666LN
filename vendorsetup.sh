...
git apply --reverse --check ../../device/itel/S66LN/patches/0001-Change-haptics-activation-file-path.patch || REVERSE=$?
cd ../../
...
if [ -f "$TFILE" ];then
    ...
    cd bootable/recovery
    git apply ../../device/itel/S66LN/patches/0001-Change-haptics-activation-file-path.patch || RET=$?
    cd ../../
    if [ $RET -ne 0 ];then
